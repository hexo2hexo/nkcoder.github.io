title: Scrapy爬虫入门实例
categories: Backend
keywords: python, scrapy, cralwer
date: 2015-12-10 16:20:48
tags: Scrapy
---

在搭建好了Scrapy的开发环境后（如果配置过程中遇到问题，请参考上一篇文章
[搭建Scrapy爬虫的开发环境](http://nkcoder.github.io/2015/11/17/Scrapy-crawl-intro-install-and-config/)，
或者在博客里留言），我们开始演示爬取实例。

我们试图爬取[论坛-东京版](http://www.xiaochuncnjp.com/forum-19-1.html)的主题贴。该网
...
然后使用Scrapy提供`FormRequest.from_response()`模拟页面的登录过程，主要代码如下：
...
以，另外[BeautifulSoup](http://www.crummy.com/software/BeautifulSoup/bs4/doc/)
分析HTML/XML文件非常方便，这里采用XPath分析页面，请参考
[zvon-XPath 1.0 Tutorial](http://zvon.org/comp/r/tut-XPath_1.html)，示例丰富且易
...
首先我们需要定义一个decorator，表示如果spider的`pipeline`属性中包含了添加该注解的pipeline，
...
数据存储的逻辑在pipeline中实现，可以使用`twisted adbapi`以线程池的方式与数据库交互。首
...
`bucket.fetch()`接口，因为经常上传失败，建议使用`put_data()`接口，比如：
...
ok，这篇入门实例的重点就这么多，项目的源码[在gitlab](https://gitlab.com/daniel-blogcode/scrapy_in_practice)。
